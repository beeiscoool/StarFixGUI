...
The system integrates game logic, user interaction and real-time feedback using a graphical interface which makes the gameplay interactive and engaging.
...
- Questions are organized into themed levels. (Easy and Hard)
...
- Option to proceed to the next level or exit the game.
...
Exception Handling improves system robustness using try-catch blocks. In the Program class, the application startup is wrapped in a try-catch block to handle unexpected errors without crashing. Additionally, input validation and null-checks (such as in the Questions property setter) ensure the game remains stable during user interaction.
